...
export async function createAiLessonFromGeneratedPassage(params: {
  ...
  const { error: lessonPassageError } = await supabase
    ...
    .insert({
      ...
      audio_start_ms: passage.audio_start_ms ?? null,
      audio_end_ms: passage.audio_end_ms ?? null,
      audio_sentence_start_index: passage.audio_sentence_start_index ?? null,
      audio_sentence_end_index: passage.audio_sentence_end_index ?? null,
      audio_sentence_timings: passage.audio_sentence_timings ?? null,
      audio_alignment_confidence: passage.audio_alignment_confidence ?? null,
      audio_alignment_method: passage.audio_alignment_method ?? null,
      is_primary: true,
      display_order: 1,
    });
